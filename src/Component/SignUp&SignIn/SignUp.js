  ...
  async function handleSubmit(event) {
   ...
   let max  = Math.max(...users.map(({ UID }) => UID))
      ...
      let resultdata = await fetch(
        `http://localhost:4500/add?UID=${regdata.UID}&FName=${regdata.firstName}&LName=${regdata.lastName}&Gender=${regdata.gender}&State=${regdata.state}&City=${regdata.city}&Email=${regdata.email}&Mobile=${regdata.mobile}&Password=${regdata.password}&Status=false&Like=false&Dislike=false`,
        requestOptions
      );
